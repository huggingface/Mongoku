...
export class ExploreComponent implements OnInit {
  ...
  update(upd: boolean = true) {
    ...
    const sort = this.params.sort !== "" ? JSON.stringify(this.jsonParser.parse(this.params.sort)) : "{}";
    const project = this.params.project !== "" ? JSON.stringify(this.jsonParser.parse(this.params.project)) : "{}";
    if (!query || !sort) {
      return;
    }
    ...
    // Load Content
    this.mongoDb
      .query(this.server, this.database, this.collection, query, project, sort, this.params.skip, this.params.limit)
      .subscribe((res: any) => {
    ...
    // Count documents
    this.mongoDb.count(this.server, this.database, this.collection, query).subscribe((res: any) => {
  ...
  editDocument(_id, json) {
    ...
    this.mongoDb.update(this.server, this.database, this.collection, oldId, json, partial).subscribe((res: any) => {
      if (res.ok) {
  ...
  remove(_id) {
    ...
    this.mongoDb.remove(this.server, this.database, this.collection, document).subscribe((res: any) => {
